/**
 * @param {string} base64String base64 encoded string
 * @returns {Uint8Array}
 */
export function base64ToUint8Array(base64String: string): Uint8Array {
  let binaryString = window.atob(base64String);
  let len = binaryString.length;
  let bytes = new Uint8Array(len);

  for (let i = 0; i < len; i++) {
    bytes[i] = binaryString.charCodeAt(i);
  }
  return bytes;
}

/**
 * https://www.w3.org/TR/eme-initdata-cenc/
 * @param {Uint8Array} binary data from the URI in the manifest
 * @returns {Uint8Array}
 */
export function buildPlayReadyPSSHBox(binary: Uint8Array): Uint8Array {
  // https://dashif.org/identifiers/protection/ playready uuid: 9a04f079-9840-4286-ab92-e65be0885f95
  const uuid = ['0x9A04F079', '0x98404286', '0xAB92E65B', '0xE0885F95'];
  const psshBoxType = '0x70737368';
  const psshBoxVersion = 0;
  const uriData = binary;

  // create an ArrayBuffer with size of uriData and 32 bytes in padding
  const psshArray = new ArrayBuffer(uriData.length + 32);

  // create a data view with PSSH array buffer
  const data = new DataView(psshArray);

  // convert to unsigned 8 bit array
  const pssh = new Uint8Array(psshArray);

  data.setUint32(0, psshArray.byteLength);
  data.setUint32(4, Number(psshBoxType));
  data.setUint32(8, psshBoxVersion);
  data.setUint32(12, Number(uuid[0]));
  data.setUint32(16, Number(uuid[1]));
  data.setUint32(20, Number(uuid[2]));
  data.setUint32(24, Number(uuid[3]));
  data.setUint32(28, uriData.length);

  pssh.set(uriData, 32);

  return pssh;
}

/*
 * @param {ArrayBuffer} keyMessage
 * @returns {Array<[string, string]>} playReadyHeaders
 */
export function makePlayreadyHeaders(
  keyMessage: ArrayBuffer
): Array<[string, string]> {
  const xmlContent = String.fromCharCode.apply(
    null,
    new Uint16Array(keyMessage)
  );
  const parser = new window.DOMParser();
  const keyMessageXml = parser.parseFromString(xmlContent, 'application/xml');
  const headers = keyMessageXml.querySelectorAll('HttpHeader');
  const playReadyHeaders: Array<[string, string]> = [];

  let header;

  for (let i = 0, len = headers.length; i < len; i++) {
    header = headers[i];
    playReadyHeaders.push([
      header.querySelector('name').textContent,
      header.querySelector('value').textContent,
    ]);
  }

  return playReadyHeaders;
}
